...
export default function PlayerVote({ room, sessionId }: PhaseComponentProps) {
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 p-4">
        <motion.div
          ...
          className="text-5xl"
        >
          <Paintbrush className="h-12 w-12" style={{ color: "var(--color-scrawl)" }} />
        </motion.div>
        <p className="font-display text-2xl font-bold">{da.scrawl.artistWaiting}</p>
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-6 p-4">
      ...
      <p className="font-display text-xl font-bold">{da.scrawl.guessReal}</p>
