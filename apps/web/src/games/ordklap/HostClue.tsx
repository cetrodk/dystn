...
export default function HostClue({ room }: PhaseComponentProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-10">
      ...
      <div className="text-8xl font-mono font-bold text-[var(--color-ordklap)] glow-text">
      ...
      <div className="flex flex-col items-center gap-3">
        {clueGiver && (
          <motion.div
            ...
            className="flex items-center gap-3 rounded-2xl bg-[var(--color-ordklap)]/10 ring-1 ring-[var(--color-ordklap)]/30 px-6 py-3"
          >
            <GameAvatar name={clueGiver.name} avatarColor={clueGiver.avatarColor} avatarImage={clueGiver.avatarImage} className="h-12 w-12" />
            <div>
              <p className="text-lg font-bold" style={{ color: "var(--color-ordklap)" }}>{clueGiver.name}</p>
        ...
        <AnimatePresence>
          {room.players?.map((p) => (
            <motion.div
              ...
              layout
              animate={{
                backgroundColor: p._id === clueGiverId ? "var(--color-ordklap)" : "var(--color-surface)",
