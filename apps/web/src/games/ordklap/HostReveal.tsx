  ...
  return (
    <div className="flex flex-col items-center gap-8">
      ...
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="rounded-xl bg-[var(--color-ordklap)]/10 px-6 py-2"
      >
        <span className="text-sm text-[var(--color-text-muted)]">{da.ordklap.clueIs}: </span>
      ...
      <AnimatePresence>
        {showTarget && (
          <motion.p
            ...
            className="text-lg text-[var(--color-ordklap)] font-bold"
          >
            {da.ordklap.targetWas} {target}
          </motion.p>
        )}
      </AnimatePresence>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-[var(--color-text-muted)]">
                  {result.guess != null
                    ? `Gættede ${result.guess} (${result.distance === 0 ? da.ordklap.exact : result.distance === 1 ? da.ordklap.close : result.distance === 2 ? da.ordklap.near : da.ordklap.miss})`
                    : "Intet gæt"}
                </p>
              </div>
              <motion.span
                ...
                className={`font-display text-3xl font-bold ${
                  result.score > 0 ? "text-[var(--color-ordklap)]" : "text-[var(--color-text-muted)]"
                }`}
              >
                +{result.score}
              </motion.span>
      ...
      <AnimatePresence>
        {stage === "done" && (
          ...
          >
            <div className="rounded-2xl bg-[var(--color-ordklap)]/10 ring-1 ring-[var(--color-ordklap)]/30 px-8 py-4 text-center">
              <p className="text-sm uppercase tracking-widest text-[var(--color-text-muted)]">
                {da.ordklap.clueGiverBonus}
              </p>
              <p className="text-lg font-semibold">{clueGiverName}</p>
              <p className="font-display text-3xl font-bold text-[var(--color-ordklap)]">+{clueGiverBonus}</p>
            </div>

            <button
              onClick={() => send({ type: "hostAdvance", hostId: sessionId })}
              className="mt-4 rounded-2xl bg-[var(--color-ordklap)] px-12 py-5 text-2xl font-bold text-[#0d0b1a] transition-transform hover:scale-105 active:scale-95 cursor-pointer"
            >
