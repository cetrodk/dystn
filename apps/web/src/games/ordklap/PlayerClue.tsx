    ...
    return (
      <WaitingScreen deadline={room.phaseDeadline} players={room.players}>
        <p className="text-sm text-[var(--color-text-muted)]">
          {clueGiver?.name} {da.ordklap.waitingForClue}
        </p>
      </WaitingScreen>
    );
  }
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-6 p-4">
      ...
      <motion.p
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="font-display text-xl font-bold text-[var(--color-ordklap)]"
      >
        {da.ordklap.youAreClueGiver}
      </motion.p>
      ...
      <form onSubmit={handleSubmit} className="flex w-full max-w-xs flex-col gap-4">
        <input
          ...
          placeholder={da.ordklap.cluePlaceholder}
          className="w-full rounded-xl bg-[var(--color-surface)] p-4 text-center text-lg placeholder:text-[var(--color-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-ordklap)]/50"
          autoComplete="off"
          autoFocus
        />
        <button
          type="submit"
          disabled={!clue.trim()}
          className="rounded-xl bg-[var(--color-ordklap)] p-4 text-xl font-bold text-[#0d0b1a] transition-transform hover:scale-105 active:scale-95 disabled:opacity-40 cursor-pointer"
        >
          {da.submit}
        </button>
      </form>
