    ...
    return (
      <WaitingScreen deadline={room.phaseDeadline} players={room.players}>
        <p className="text-sm text-[var(--color-text-muted)]">
          {da.hunch.watchingGuesses}
        </p>
        <p className="text-sm text-[var(--color-text-muted)]">
          {da.hunch.clueIs}: <span className="font-bold text-[var(--color-text)]">{clue}</span>
        </p>
        <p className="text-sm text-[var(--color-text-muted)]">
    ...
    <div className="flex min-h-screen flex-col items-center justify-center gap-6 p-4">
      ...
      <div className="w-full max-w-xs">
        ...
        <input
          ...
          className="w-full accent-[var(--color-hunch)] h-3 cursor-pointer"
        />
        <div className="flex justify-between text-xs text-[var(--color-text-muted)] mt-1 px-1">
          <span>1</span>
          <span className="font-bold text-[var(--color-hunch)] text-lg">{position}</span>
          <span>10</span>
        </div>
      </div>
      ...
      <button
        ...
        className="rounded-xl bg-[var(--color-hunch)] px-8 py-4 text-xl font-bold text-[#0d0b1a] transition-transform hover:scale-105 active:scale-95 cursor-pointer"
      >
        {da.hunch.submitGuess}
      </button>
    </div>
  );
}
